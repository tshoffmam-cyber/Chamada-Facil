...
return (
<button key={iso} onClick={()=>setSelecionado(iso)} style={{flexShrink:0,minWidth:52,display:'flex',flexDirection:'column',alignItems:'center',gap:2,padding:'10px 8px',borderRadius:16,background: ativo ? 'var(--color-primary)' : 'var(--color-surface)',border: ativo ? 'none' : (ehFeriado ? '1px solid var(--color-warning)' : '1px solid var(--color-border)'),color: ativo ? '#fff' : 'var(--color-text-primary)',cursor:'pointer',position:'relative'}}>
<span style={{fontSize:11,fontWeight:600}}>{DIAS_SEMANA[d.getDay()]}</span>
<span style={{fontSize:16,fontWeight:800}}>{d.getDate()}</span>
{ehFeriado && <div style={{width:5,height:5,borderRadius:'50%',background: ativo ? '#fff' : 'var(--color-warning)',marginTop:1}}/>}
</button>
)
})}
</div>
