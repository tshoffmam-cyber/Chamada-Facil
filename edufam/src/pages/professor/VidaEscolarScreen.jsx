...
import { mockAlunos, mockVidaEscolar } from '../../data/mockData'
...
export default function VidaEscolarScreen() {
  ...
  const registros = mockVidaEscolar
    ...
    <div style={{ paddingBottom: 24 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
          <button
            onClick={() => navigate(`/aluno/${alunoId}`)}
            style={{
              width: 36, height: 36,
          ...
          <div>
            ...
            <p style={{ fontSize: 13, color: 'var(--color-text-secondary)', margin: 0 }}>{aluno.nome}</p>
          </div>
          <button
      ...
      {showForm && (
        <div style={{ padding: '16px 20px', background: 'var(--color-surface)', borderBottom: '1px solid var(--color-border)' }}>
          <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 12 }}>Novo Registro</h3>
      ...
      <div style={{ padding: '16px 20px 0' }}>
        {registros.length === 0 ? (
          <div className="empty-state">
            <div className="empty-state-icon">timeline</div>
              ...
              return (
                <div key={v.id} style={{ display: 'flex', gap: 16, marginBottom: 16, position: 'relative' }}>
                  <div style={{
                    ...
                    {v.tipo === 'registro_positivo' ? 'star' : v.tipo === 'ocorrencia' ? 'warning' : 'task_alt'}
                  </div>
